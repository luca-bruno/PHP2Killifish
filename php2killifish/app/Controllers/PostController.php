...
class PostController extends BaseController {
    ...
    function view($postSlug = NULL){ // different posts may be viewed according to their slug, used as their URL extension
        ...
        $data['posts'] = $model->getPosts($postSlug); // all posts with matching post slug
        
        $data['comments'] = $commentModel->getComments(); // all comments within post

        if (empty($data['posts'])){ // if no posts match post slug
        ...
        if ($this->request->getMethod() == 'post'){ // built-in CI4 validation rules for users creating a new post
            ...
            if (! $this->validate($rules)){ // if form is not valid
                ...
                $newData = [
                    ...
                    'postSlug' => url_title($this->request->getPost('postTitle')) // creates URL slug from postTitle
                ];  // send our data to model
                $model->save($newData); // save it
                $id = $model->getInsertID();// insert id of the last post
                $this->uploadImage($id);
                ...
                return redirect()->to('community'); // return user to community page
            }
        }
